...
pub async fn get_authorization_url(
    ...
    session
        .insert("CSRF_TOKEN", csrf_token.secret().to_owned())
        .await
...
>(
    ...
    let response = callback::<T, U, V>(ctx, session, params, jar, &mut client).await?;
    drop(client);
    Ok(response)
}
...
>(
    ...
    let user = callback_jwt::<T, U, V>(&ctx, session, params, &mut client).await?;
    drop(client);
    let token = user
